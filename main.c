    ...
    for(i = 1; i <= number_of_node; i++)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                fprintf(file,"\n\tS.L.C Passing Year: %d",new_node->hsc_passing_year);

                printf("\n\tS.L.C Result in GPA: ");
                scanf("%f", &new_node->hsc_gpa);
                fprintf(file,"\n\tS.L.C Result in GPA: %0.2f",new_node->hsc_gpa);

                printf("\n\n\tS.L.C Board: ");
                fflush(stdin);
                gets(new_node->hsc_board);
                fprintf(file,"\n\tS.L.C Board: %s", new_node->hsc_board);
                ...
                fprintf(file,"\n\tN.E.B Passing Year: %d",new_node->ssc_passing_year);

                printf("\n\tN.E.B Result in GPA: ");
                scanf("%f", &new_node->ssc_gpa);
                fprintf(file,"\n\tN.E.B Result in GPA: %0.2f",new_node->ssc_gpa);

                printf("\n\n\tN.E.B Board: ");
                fflush(stdin);
                gets(new_node->ssc_board);
                fprintf(file,"\n\tN.E.B Board: %s", new_node->ssc_board);
...
void display_students_record_from_forward()
{
    ...
    else
    {
        while(current != NULL)
        {
            ...
            printf("\n\tS.L.C Passing Year: %d", current->hsc_passing_year);

            printf("\n\tS.L.C Result in GPA: %0.2f", current->hsc_gpa);

            printf("\n\tS.L.C Board: %s", current->hsc_board);

            printf("\n\tSchool Name: %s", current->school_name);

            printf("\n\tN.E.B Passing Year: %d", current->ssc_passing_year);

            printf("\n\tN.E.B Result in GPA: %0.2f", current->ssc_gpa);

            printf("\n\tN.E.B Board: %s", current->ssc_board);
